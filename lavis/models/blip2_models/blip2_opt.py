...
class Blip2OPT(Blip2Base):
    ...
    def __init__(
        ...
        self.Qformer, self.query_tokens, _ = self.init_Qformer(
    ...
    def forward(self, samples):
        ...
        if image.dim() == 4:
            ...
            tmp_list = []
            for adj_img in image[:,:-1,...].split(1, dim=1):
            ...
            image_atts = torch.ones(tmp_query.size()[:-1], dtype=torch.long).to(image.device) # [2, 128]
        ...
        with torch.cuda.amp.autocast(dtype=torch.float32):
            ...
            b, vis_len = atts_opt.size()
            b, q_len = input_tokens.input_ids.shape
            empty_targets = (
                torch.ones((b, vis_len + q_len), dtype=torch.long).to(image.device).fill_(-100)
            )
            targets = torch.cat([empty_targets, targets], dim=1)

            inputs_embeds = self.opt_model.model.decoder.embed_tokens(input_tokens.input_ids)
            target_embeds = self.opt_model.model.decoder.embed_tokens(output_tokens.input_ids)
            inputs_embeds = torch.cat([inputs_opt, inputs_embeds, target_embeds], dim=1)
            attention_mask = torch.cat([atts_opt, input_tokens.attention_mask, output_tokens.attention_mask], dim=1)
