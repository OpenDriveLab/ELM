...
@registry.register_model("blip2_vqa_t5")
class Blip2VQAT5(Blip2Base):
    """
    BLIP2 T5 model.
    ...
    def __init__(
        ...
        self.Pickformer, _, self.pick_tokens = self.init_Qformer(32, 768, temporal_length=temporal_length)
        self.Pickformer.cls = None
        self.Pickformer.bert.embeddings.word_embeddings = None
        self.Pickformer.bert.embeddings.position_embeddings = None
        for layer in self.Pickformer.bert.encoder.layer:
            layer.output = None
            layer.intermediate = None
    ...
    def forward(self, samples):
        image = samples["image"]
        adj_images = samples["adj_images"]
        scene_id = samples["scene_id"]
        adj_id = samples["adj_id"]
        B = image.shape[0]
        # print(scene_id,adj_id)
        # print(self.memory_bank.keys())
        # input()
        ...
        #     for adj_img in adj_images.split(1, dim=1):
        #         adj_img = adj_img.squeeze(1)
        #         adj_img_embeds = self.ln_vision(self.visual_encoder(adj_img))
        #         image_embeds = torch.cat([image_embeds, adj_img_embeds], dim=1)
        ...
        with self.maybe_autocast():
            ...
            with torch.no_grad():
                mem_img_embeds = self.find_adj(B, adj_id, scene_id, image_embeds)
            # mem_img_embeds = None
            # print(mem_img_embeds)
            # input()

            
            if mem_img_embeds != None:
                mem_img_embeds = mem_img_embeds.to(image.device)
                # print("yes")
                # print(mem_img_embeds.shape)

            i = 0
            # fuse_query_tokens = self.pick_tokens.expand(2, -1, -1)
            # adj_query_token = self.extra_query_tokens.expand(2, -1, -1)

            for adj_img in adj_images.split(1, dim=1):
                adj_img = adj_img.squeeze(1)
                # adj_img_embeds = self.ln_vision(self.visual_encoder(adj_img))
                if mem_img_embeds != None:
                    adj_img_embeds = mem_img_embeds[:,i,...]
                else:
                    adj_img_embeds = image_embeds

                # adj_img_embeds = image_embeds

                adj_img_atts = torch.ones(adj_img_embeds.size()[:-1], dtype=torch.long).to(image.device)

                adj_query_token = self.extra_query_tokens.expand(adj_img_embeds.shape[0], -1, -1)
                adj_query_output = self.Qformer.bert(
                    query_embeds=adj_query_token,
                    encoder_hidden_states=adj_img_embeds,
                    encoder_attention_mask=adj_img_atts,
                    return_dict=True,
                )

                if i == 0:
                    mem_query = adj_query_output.last_hidden_state

                else:
                    mem_query = torch.cat([mem_query, adj_query_output.last_hidden_state], dim=1)
                    mem_atts = torch.ones(mem_query.size()[:-1], dtype=torch.long).to(image.device)
                    ...
                    # print(mem_query.shape)
                i += 1
            # mem_atts = torch.ones(mem_query.size()[:-1], dtype=torch.long).to(image.device)
            t5_query = torch.cat([query_output.last_hidden_state, mem_query], dim=1)
           
           
            # else:
            #     t5_query = query_output.last_hidden_state

                # query_output = mem_query



                # adj_inputs_t5 = self.t5_proj(query_output.last_hidden_state)
                # # adj_inputs_t5 = inputs_t5
                # inputs_t5 = torch.cat([inputs_t5, adj_inputs_t5], dim=1)
                # # inputs_t5 = adj_inputs_t5


        # inputs_t5 = self.t5_proj(torch.cat([mem_query, query_output.last_hidden_state], dim=1))
