...
@registry.register_model("blip2_vqa_t5_baseline")
class Blip2VQAT5BASELINE(Blip2Base):
    """
    BLIP2 T5 model.
    ...
    def timestep_embedding(self, timesteps, dim, max_period=1000, repeat_only=False):
        """
        ...
        with self.maybe_autocast():
            if vfeats.dim() == 4:
                ...
                for adj_img in vfeats.split(1, dim=1):
                    ...
                    t5_query = query_output.last_hidden_state
                    t5_query_list.append(t5_query)

                t5_query = torch.cat(t5_query_list, dim=1)
