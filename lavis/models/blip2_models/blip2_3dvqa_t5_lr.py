...
class Blip2VQAT5LR(Blip2Base):
    ...
    def forward(self, samples):
        ...
        with self.maybe_autocast():
            if vfeats.dim() == 4:
                ...
                if "timesteps" in samples:
                    ...
                    Time = vfeats.shape[1]
                    timesteps = torch.tensor(np.arange(-0.5*Time,0,0.5)).to(device).unsqueeze(0).expand(vfeats.shape[0], Time)
                tmp_list = []
        ...
        with self.maybe_autocast():
            ...
            text_tmp_query = tmp_query.reshape(B, self.temporal_length, -1, 1408)
    ...
    def from_config(cls, cfg):
        ...
        model.learnable_query.requires_grad = True
        
        for param in model.cross_question.parameters():
            param.requires_grad = True
        for param in model.cross_img.parameters():
            param.requires_grad = True
        for param in model.alignimg.parameters():
            param.requires_grad = True
        return model
