...
import copy, random
import math
from lavis.models.blip2_models.osrt.layers import SparseAttention
from peft import LoraConfig, get_peft_model, TaskType


@registry.register_model("blip2_vqa_t5_lrtime")
class Blip2VQAT5LRTIME(Blip2Base):
    """
    BLIP2 T5 model.
    ...
    def __init__(
        ...
        self.slot_attention = SparseAttention(num_slots=32, input_dim=1408, slot_dim=1408, iters=1,
                                            randomize_initial_slots=False)
        self.learnable_query = nn.Parameter(torch.rand(1, 1408))
    ...
    def add_pos_embed(self, t5_query, timesteps):
        ...
        for i in range(t5_query.shape[0]):
            emb_te = self.leanerable_pos_embed(timesteps[i])
    ...
    def leanerable_pos_embed(self, timesteps, max_period=1000, repeat_only=False):
        # expand the 2048 to B x 2048
        learnable_query = self.learnable_query.expand(timesteps.shape[0], -1)
        learnable_query = self.mlp_network(learnable_query)
        
        return learnable_query
    ...
    def from_config(cls, cfg):
        ...
        model.mlp_network.requires_grad = True
